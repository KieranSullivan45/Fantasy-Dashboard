...
- Status: in-progress (phase A contracts ready; awaiting owner confirmation before V04-02 starts; phase B not started)
- Owner: Claude Code
...
- Handoff notes (phase A — **contracts ready**): V04-PLANNING merged (PR #2); owner confirmed the assignment and approved the behavior-preserving `add-drop.js` protection-evidence helper (ADR 0001 question 7), which is deferred to phase B because phase A does not need it. Added `lib/trade/contracts.js`: JSDoc types for `TradeContext`/`TradeProposal`/`TradeEvaluation`, the `next_game` horizon object (other horizons `supported: false`), error codes with status classes and precedence, canonical id order, `validateTradeProposal` (whole-package structural validation, plus roster and ownership checks when given a context), `validateTradeContext`, `effectiveProtection` (reads authoritative records only; anything missing, malformed or with mismatched provenance is `insufficient`), `checkBasis` (existing basis equality, unchanged), `compareContextMetadata` (trade-local `CONTEXT_MISMATCH`), `slotStructure`, `capabilityAvailable`, and evaluation envelope builders and invariant validator (`market_value: null`, conditional legality, forbidden score/verdict/probability keys, blocked/undetermined semantics). Contract decisions and additions (`MALFORMED_PROPOSAL`, `INVALID_CONTEXT`, `legality_limitations`, field names) are recorded in `docs/v0.4.md` → *Phase A contract checkpoint*. Files: `lib/trade/contracts.js` (new), `test/trade-core.test.js` (new, 16 tests), `docs/v0.4.md` (contract section), this entry. No existing code changed. Tests: `pnpm test` 114 pass / 0 fail / 0 skipped (98 existing + 16 new); `pnpm build` compiled successfully; `pnpm test:ui` not run (no UI, routes or loaders touched). Remaining (phase B): add-drop helper and equivalence test, context builder, forced drops, evaluator, matrix rows marked V04-01. Open questions for the reviewer: (1) the spec says provenance mismatch is both `CONTEXT_MISMATCH` (context validation) and per-player `insufficient` (evidence section); phase A resolves this as bundle-level mismatch ⇒ `CONTEXT_MISMATCH` and per-record mismatch ⇒ `insufficient`. (2) Without a current week (e.g. preseason), the context is `INVALID_CONTEXT` / `CONTEXT_MISMATCH` rather than having a dedicated code.
- Blockers: owner confirmation of the phase A contracts before V04-02 starts.

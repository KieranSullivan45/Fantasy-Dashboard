...
export default function PlayersView() {
  ...
  const engaged = Boolean(filters.query.trim() || position !== "ALL" || filters.availability !== "ALL" || filters.browse);
  ...
  return <><h1>Players</h1><DecisionPending />{playerId ? <><Link href="/dashboard/players">← Back to player results</Link>{selected ? <article className="card playerDetail"><h2>{selected.name}</h2><p>{(selected.fantasy_positions || [selected.position]).join("/")} · {owners.get(playerId) || "Available in this league"}</p><PlayerQuickContext context={context} /><details open><summary>Player analytics and evidence</summary>{context ? <PlayerContextCard context={context} /> : <p>Decision context unavailable for this returned player.</p>}</details><details><summary>Active signals</summary>{decision?.signals.records.filter(s => s.player_id === playerId).map(s => <div key={s.signal_id}><strong>{s.type.replaceAll("_"," ")}</strong><p>{s.explanation} · {s.confidence} · {s.sample_size} games · through W{s.data_through_week}</p><pre className="evidenceJson">{JSON.stringify(s.evidence,null,2)}</pre>{s.warnings.map(w => <p key={w}>{w}</p>)}</div>)}</details></article> : <p className="card">Player not found in this league's returned data. The link is preserved; choose another league or return to search.</p>}</> : <>
    ...
    {!engaged ? <div className="quickLinks"><button onClick={() => change({availability:"AVAILABLE"})}>Available players</button><button onClick={() => change({availability:"ROSTERED"})}>Rostered players</button><button onClick={() => change({browse:true})}>Browse returned players</button><Link href="/dashboard/signals">Recent signals / role risers</Link></div> : null}
    <p className="muted">{engaged ? results.length : 0} matches in {pool.length} returned players; this is not the complete NFL catalog. Showing {Math.min(page*15+1,results.length)}–{Math.min((page+1)*15,results.length)}.</p><div className="playerResults">{(engaged ? results.slice(page*15,(page+1)*15) : []).map(p => <article className="card" key={p.player_id}><CompactPlayer player={p} context={decision?.player_context[p.player_id]} scope="search" href={`/dashboard/players?player=${encodeURIComponent(p.player_id)}`} ownership={owners.get(p.player_id) || "Available"} /></article>)}</div><div className="pager"><button disabled={!page} onClick={() => setFilters({...filters,page:page-1})}>Previous players</button><button disabled={!engaged || (page+1)*15>=results.length} onClick={() => setFilters({...filters,page:page+1})}>Next players</button></div>
  </>}</>;
}

...
export async function GET(request) {
  ...
  if ([...p.keys()].some(k => !["username", "user", "league", "season"].includes(k) || p.getAll(k).length !== 1)) return Response.json({ error: "Unknown or repeated discovery parameter" }, { status: 400 });
  try {
    ...
    const result = hasQuery ? await discoverSleeper({ username: p.get("username"), userId: p.get("user"), season: p.get("season"), leagueId: p.get("league") }) : {

export default function SourceStatus({ data }) {
  return <details className="card sourceStatus"><summary>Data sources, coverage and limitations</summary>
    ...
    <ul>{data.sources.map(source => <li key={source.source_id}><strong>{source.source_id}</strong>: {source.status}
      ...
      {source.through_week != null ? ` · through week ${source.through_week}` : ""}
    ...
    <p className="muted">D/ST and IDP historical scoring are unavailable. Snap counts are published by nflverse from PFR; expected event counts come from ffopportunity and use covered league rules. This app performs no HTML scraping. No paid providers or ownership-history storage are enabled.</p>
  </details>;
}

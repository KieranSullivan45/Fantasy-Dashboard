...
export async function handleSnapshotRequest(request, { buildSnapshot = buildLeagueSnapshot, leagueIds = getConfiguredLeagueIds() } = {}) {
  ...
  if (requested !== "all" && !leagueIds.includes(requested)) return Response.json({ error: "Unknown league id", configured_league_ids: leagueIds }, { status: 400 });
  const build = async id => {
    const snapshot = await buildSnapshot(id, { freeAgentLimit: compact ? 35 : 100 });

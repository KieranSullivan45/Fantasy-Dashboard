...
export function playerIdMap(rows) {
  const groups = new Map();
  for (const row of rows) {
    if (!row.gsis_id || !row.sleeper_id || ["NA", "null"].includes(row.sleeper_id) || ["NA", "null"].includes(row.gsis_id)) continue;
    if (!groups.has(row.gsis_id)) groups.set(row.gsis_id, new Set());
    groups.get(row.gsis_id).add(String(row.sleeper_id));
  }

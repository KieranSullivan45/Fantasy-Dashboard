...
export function createDecisionService(build = buildDecisionContext, now = Date.now) {
  ...
  return (league, options = {}) => {
    const key = JSON.stringify([options.provider || "sleeper", league, options.identity && "userId" in options.identity ? options.identity.userId : "installation-default", options.identity?.rosterId ?? null, options.identity?.season ?? null]);

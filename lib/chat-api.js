...
import { getProvider, requestProvider } from "./providers/index.js";
import { providerErrorResponse } from "./providers/contracts.js";
...
export async function handleChatRequest(request, resource, { build = decisionService, discover = options => getProvider(options.provider).discoverLeagues(options), state = () => publicSleeper("/state/nfl"), archive = readArchiveJson, configured = getConfiguredLeagueIds() } = {}) {
  const p = new URL(request.url).searchParams;
  const allowed = new Set(["provider", "league", "user", "roster", "season", "week", "username", "player", "limit", "offset", "type"]);
  ...
  let provider;
  try { provider = requestProvider(p,{live:resource !== "model-meta"}); } catch(e) { return providerErrorResponse(e); }
  ...
  const meta = { schema_version: "chat-1", provider: provider.providerId, provider_capabilities: provider.getProviderCapabilities(), model_version: MODEL_VERSION, feature_version: FEATURE_VERSION, signal_version: SIGNAL_VERSION, generated_at: new Date().toISOString(), season: null, week: null, data_through_week: null, league: null, warnings: [], evidence: { confidence: "unavailable_without_league_context" } };
  try {
    ...
    if (resource === "leagues") {
      const data = await discover({ provider: provider.providerId, username: p.get("username"), userId: p.get("user") || (!p.has("username") ? getConfiguredUsername() : null), season: p.get("season"), leagueId: p.get("league") });
    ...
    if (resource === "history") {
      ...
      const captures = (result.data?.captures || []).filter(c => (c.provider || "sleeper") === provider.providerId && c.league_id === league && (!identity.season || c.season === identity.season) && (identity.userId == null || c.provider_user_id === identity.userId) && (!identity.rosterId || c.roster_id === identity.rosterId));
    ...
    const d = await build(league, { identity, provider: provider.providerId });

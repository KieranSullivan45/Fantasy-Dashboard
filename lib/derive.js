...
export function isCurrentFantasyPlayer(raw, trendingAdds = 0) {
  if (!raw) return false;
  const positions = raw.fantasy_positions || [raw.position];
  if (!FANTASY_POSITIONS.some(pos => positions.includes(pos) || raw.position === pos)) return false;
  ...
  const injured = Boolean(raw.injury_status) || /injured|\bir\b|pup|reserve|out|questionable|doubtful/.test(status);
...
export function normalizePlayer(playerId, players = {}) {
  ...
  if (!p) {
    const defense = /^[A-Z]{2,4}$/.test(String(playerId));
    return {
...
export function buildRosterViews({ rosters = [], users = [], players = {}, userId }) {
  ...
  return rosters.map((roster) => {
    ...
    return {
      ...
      starters: mapped.filter((p) => p.starter),
...
export function buildFreeAgents({ players = {}, rosters = [], trending = [] }) {
  ...
  const groups = Object.fromEntries(FANTASY_POSITIONS.map((pos) => [pos, []]));

  for (const [playerId, raw] of Object.entries(players)) {
    if (rostered.has(playerId)) continue;
    if (!isCurrentFantasyPlayer(raw, trendMap.get(playerId) || 0)) continue;

    const positions = raw.fantasy_positions || (raw.position ? [raw.position] : []);
    const eligible = FANTASY_POSITIONS.find((pos) => positions.includes(pos) || raw.position === pos);
    if (!eligible) continue;

    const player = normalizePlayer(playerId, players);
    groups[eligible].push({
      ...player,

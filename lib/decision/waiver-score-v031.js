...
export function waiverRecommendations(candidates, contexts, strengths, myRosterId, allPlayers, limit = 20) {
  ...
  const scored = candidates.map(player => {
    ...
    const components = {
      ...
      role_opportunity: component(positions.some(p => ["QB", "RB", "WR", "TE"].includes(p)) ? best("opportunity") : null, 20, opportunity, "Recent targets + carries per recorded game, compared within platform eligibility; for QBs this is rushing involvement, not passing volume. Not applicable to K/DEF/IDP."),
    ...
    const lead = teammates.find(p => contexts[p.player_id]?.analytics?.trends?.carry_share?.recent_average >= 0.5 && contexts[p.player_id].analytics.through_week === analytics?.through_week && contexts[p.player_id].analytics.current_team_matches !== false);

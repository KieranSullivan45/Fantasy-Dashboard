...
export function buildProduction(rawRows, ids, { season, week, settings, platformPlayers = new Map() }) {
  ...
  for (const raw of rawRows) {
    ...
    const scoring = scoreStats(raw, settings, position);
    const game = { player_id, source_player_id: raw.player_id, game_id: raw.game_id, week: Number(raw.week),
      team: normalizeTeam(raw.team), opponent: normalizeTeam(raw.opponent_team), position, provider_position: raw.position,
  ...
  for (const [id, playerGames] of gamesByPlayer) {
    ...
    players[id] = { provider_positions: [...new Set(playerGames.map(g => g.provider_position))], analytics_positions: [...new Set(playerGames.map(g => g.position))], ppg: mean(playerGames), recorded_games: playerGames.length, scored_games: scored.length,

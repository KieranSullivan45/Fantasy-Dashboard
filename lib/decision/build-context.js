import { loadFantasyContext, getProvider } from "../providers/index.js";
...
import { loadHighValue, playerHighValue } from "../sources/plays/aggregates.js";
import { loadMarketHistory } from "../history/archive-reader.js";
import { SOCIAL_SOURCE_STATUS } from "../market/observations.js";
import { buildSignals } from "../signals/engine.js";

export async function buildDecisionContext(leagueId, {
  loadLeague = loadFantasyContext, statsSource = loadStats, scheduleSource = loadSchedules,
  ...
  highValueSource = loadHighValue, marketHistorySource = loadMarketHistory,
} = {}) {
  const snapshot = await loadLeague(leagueId, { ...identity, provider });
  if(snapshot.coverage?.unsupported_slots?.length)throw new ProviderError("UNSUPPORTED_FEATURE",snapshot.identity?.provider||provider,"Unsupported lineup slots prevent legal decision evaluation.");
  ...
  const [stats, schedules, ids, snaps, opportunity, priorStats, highValue, marketHistory] = await Promise.all([
    ...
    currentRegular ? optional("nflverse_high_value", () => highValueSource(season)) : { status: "unsupported", data: null, warnings: ["Current regular season required"] },
    currentRegular ? optional("market_history", () => marketHistorySource(season)) : { status: "unsupported", data: null },
  ]);
  const schedule = normalizeSchedule(schedules.data, season);
  const platform = snapshot.identity?.provider || provider;
  const adapter = getProvider(platform);
  const { rows: idRows, idColumn } = adapter.getIdentityRows(ids.data);
  const idMap = playerIdMap(idRows, idColumn);
  ...
  const analytics = buildAnalytics(stats.data, snaps.data, idRows, { season, week: currentRegular ? week : 0, production, idColumn });
  ...
  const interest = adapter.getInterest(freeAgents, snapshot.warnings);
  const contexts = Object.fromEntries([...players.values()].map(player => {
    ...
    return [player.player_id, { player: { player_id: player.player_id, ...(player.identity ? { identity: player.identity } : {}), name: player.name, position: player.position, fantasy_positions: fantasyPositions(player), provider_positions: production.players[player.player_id]?.provider_positions || [], team: player.team, injury_status: player.injury_status, status: player.status },
  ...
  // New evidence is intentionally attached AFTER football models are computed; calibrated inputs stay unchanged.
  const reverseIds = new Map([...idMap.map].map(([source, player]) => [player, source]));
  const generatedAt = new Date(now).toISOString(), signalWeek = Math.max(0, ...production.games.map(g => g.week));
  for (const context of Object.values(contexts)) {
    context.high_value = highValue.status === "available" ? playerHighValue(context, highValue.data, reverseIds.get(context.player.player_id), signalWeek) : null;
    context.market_attention = adapter.getMarketAttention(context, { season, week, generatedAt, previous: marketHistory.data?.observations || [] });
    context.schedule_outlook = [1, 2, 3].map(offset => { const game = upcomingGame(context.player.team, schedule, week + offset, now), m = adjusted[context.model.features.position]?.[game.opponent]; return { week: week + offset, opponent: game.opponent, matchup: m ? { games: m.games, adjusted_points_per_game: m.adjusted_points_per_game, label: m.label, through_week: m.through_week } : null }; });
  }
  const signals = buildSignals(contexts, { leagueId, season, week: currentRegular ? week : 0, throughWeek: signalWeek, generatedAt, modelVersion: MODEL_VERSION });
  ...
  const selected = new Set([...snapshot.rosters.flatMap(r => r.all_players.map(p => p.player_id)), ...snapshot.matchup_players.map(p => p.player_id),
    ...
    ...waivers.recommendations.map(p => p.player_id), ...waivers.limited_candidates.map(p => p.player_id), ...Object.values(waivers.categories).flatMap(c => c.players.map(p => p.player_id)), ...signals.records.map(s => s.player_id)]);
  ...
  const { data: highData, ...highStatus } = highValue;
  sources.push({ ...highStatus, source_id: "nflverse_high_value", records: highData?.records?.length || 0, through_week: highData?.data_through_week ?? null });
  if (highValue.status === "available" && highData.data_through_week < signalWeek) warnings.push(`High-value opportunity aggregates lag statistics: week ${highData.data_through_week} vs ${signalWeek}.`);
  for (const source of sources.filter(s => s.status === "unavailable")) warnings.push(`${source.source_id}: ${source.warnings.join(" ")}`);
  return { schema_version: "0.3", ...(platform !== "sleeper" ? { provider: platform, provider_capabilities: adapter.getProviderCapabilities() } : {}), model_version: MODEL_VERSION, feature_version: FEATURE_VERSION, implementation_revision: process.env.VERCEL_GIT_COMMIT_SHA || null, data_through_week: throughWeek, generated_at: new Date(now).toISOString(),
    ...
    basis: decisionBasis(snapshot), identity: snapshot.identity ?? { provider: "sleeper", provider_user_id: null, mode: "spectator" }, my_roster_id: snapshot.my_roster?.roster_id ?? null,
    ...
    matchup_difficulty: difficulty, adjusted_matchup_difficulty: adjusted, team_environment: analytics.environment, team_strength: strengths, team_strength_v2: strengthV2, waivers, sources, signals,
    market_sources: [{ source: "sleeper", status: platform === "sleeper" ? "available" : "unsupported", history_status: platform === "sleeper" ? marketHistory.status : "unsupported" }, ...SOCIAL_SOURCE_STATUS],
    history: { schema_version: "observation-1", capture_mode: "scheduled installation leagues only; GET requests never write", storage: "git-archive", status_endpoint: "/api/chat/history" }, partial: warnings.length > 0, warnings,
    analytics_coverage: { unmapped_snap_rows: analytics.unmapped_snap_rows, unavailable_metrics: [...(highValue.status === "available" ? [] : ["red_zone_opportunities"]), "end_zone_targets", "success_rate"], signal_minimum_games: 4, opportunity_source: opportunity.status },

...
import { sleeperAttention, SOCIAL_SOURCE_STATUS } from "../market/observations.js";
...
export async function buildDecisionContext(leagueId, {
  ...
  const snapshot = await loadLeague(leagueId, identity);
  ...
  for (const context of Object.values(contexts)) {
    ...
    context.market_attention = sleeperAttention(context, { season, week, generatedAt, previous: marketHistory.data?.observations || [] });
  ...
  return { schema_version: "0.3", model_version: MODEL_VERSION, feature_version: FEATURE_VERSION, implementation_revision: process.env.VERCEL_GIT_COMMIT_SHA || null, data_through_week: throughWeek, generated_at: new Date(now).toISOString(),
    ...
    market_sources: [{ source: "sleeper", status: "available", history_status: marketHistory.status }, ...SOCIAL_SOURCE_STATUS],

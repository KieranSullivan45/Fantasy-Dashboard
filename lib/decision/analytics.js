...
export function buildAnalytics(rawRows, snapRows, idRows, { season, week, production }) {
  const gsis = playerIdMap(idRows).map;
  const pfr = playerIdMap(idRows.map(row => ({ ...row, gsis_id: row.pfr_id }))).map;

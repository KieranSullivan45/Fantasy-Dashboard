...
export function transactionEvaluator(roster, rosterPositions, contexts, levels) {
  ...
  const protectedPlayers = active.map(player => {
    ...
    if (f?.current_games < 4 && f?.prior?.ppg == null) reasons.push("Unestablished asset: fewer than four games and no prior-season baseline");
    ...
    if (f?.provisional_role_expansion || f?.role_change || c?.analytics?.signals?.some(s => ["Role expanding", "Target share rising", "Backfield takeover watch"].includes(s.label))) reasons.push("Expanding-role stash");

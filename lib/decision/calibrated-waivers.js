...
export function calibratedWaivers(candidates, contexts, strengths, myRosterId, allPlayers, limit, snapshot, levels) {
  ...
  const scored = old.all_evaluations.map(item => {
    ...
    const result = { ...item, components, roster_value: tx, pickup_rating: rated ? Math.round(subtotal * 10) / 10 : null,
      ...
      comparable_group: Object.entries(components).filter(([, c]) => c.score != null && c.weight).map(([key]) => key).join("+") +
        `+role_inputs_${Object.entries(f?.opportunity.components || {}).filter(([, c]) => c.score != null).map(([key]) => key).join("_")}+forecast_inputs_${(model?.pickup_value.inputs || []).map(v => v == null ? 0 : 1).join("")}`,

...
export function highValueOpportunities(rows, { season, beforeWeek }) {
  ...
  for (const r of rows) {
    ...
    if (!games.has(gameKey)) games.set(gameKey, { player_id: id, game_id: r.game_id, season, week: Number(r.week), team: r.posteam,
      red_zone_carries: 0, inside_10_carries: 0, inside_5_carries: 0, red_zone_targets: 0, inside_10_targets: 0, end_zone_targets: null, evidence: [] });
    ...
    else { g.red_zone_targets++; if (yardline <= 10) g.inside_10_targets++; }
    const highKey = `high:${r.posteam}:${r.game_id}`; totals.set(highKey, (totals.get(highKey) || 0) + 1);
    if (rushing) { const carryKey = `carry:${r.posteam}:${r.game_id}`; totals.set(carryKey, (totals.get(carryKey) || 0) + 1); }
  ...
  return [...games.values()].map(g => ({ ...g, red_zone_opportunities: g.red_zone_carries + g.red_zone_targets, high_value_opportunities: g.red_zone_carries + g.red_zone_targets,
    high_value_opportunity_share: (g.red_zone_carries + g.red_zone_targets) / totals.get(`high:${g.team}:${g.game_id}`),
    red_zone_carry_share: totals.get(`carry:${g.team}:${g.game_id}`) ? g.red_zone_carries / totals.get(`carry:${g.team}:${g.game_id}`) : null,

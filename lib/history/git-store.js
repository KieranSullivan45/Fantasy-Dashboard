...
export async function appendCapture(directory, decision) {
  ...
  const folder = join(directory, "observations", season, league), file = `${captureId}.json.gz`;
  ...
  const entry = { capture_id: captureId, generated_at: stored.generated_at, season: Number(season), week: decision.league.week, league_id: league,
    ...
    observation_count: stored.observations.length, path: `observations/${season}/${league}/${file}`, bytes: bytes.length, sha256: digest(stored.observations),
